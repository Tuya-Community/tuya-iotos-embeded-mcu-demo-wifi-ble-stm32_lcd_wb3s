...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_ILLUMINANCE, DP_TYPE_VALUE},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_bool_update(DPID_SWITCH_1,当前开关1); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_2,当前开关2); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_3,当前开关3); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_4,当前开关4); //BOOL型数据上报;
    mcu_dp_value_update(DPID_TEMPERATURE,当前温度); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY,当前湿度); //VALUE型数据上报;
    mcu_dp_value_update(DPID_ILLUMINANCE,当前光照度); //VALUE型数据上报;
    */
}
